...
try:
	...
	while True:
		...
		while(condicion):
			...
			masaConc 		= 0#UberCow.leerConcentrado(3)
			...
			print("%s\t%s\t%s\t%s"%(str(elapsed),str(masaConc),str(masaMin),str(masaLev)))
			...
			if(not(condicionConc)and(not(condicionMin))and(not(condicionLev))):
				...
				print("\tDosifcado")
				break"""
			if ((elapsed>=10) and (elapsed<=60)):
				#UberCow.encenderMotores('Con')
				UberCow.encenderMotores('Min')
				UberCow.encenderMotores('Lev')
			else:
				UberCow.apagarMotores('Con',False)
				UberCow.apagarMotores('Min',False)
				UberCow.apagarMotores('Lev',False)
