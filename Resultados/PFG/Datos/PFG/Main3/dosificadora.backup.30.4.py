...
class Dosificadora:
	def __init__(self):
		...
		self.calcularDeltaReferencia()
		self.aInt				= [0.0,	0.0,	0.0]
		self.aKp				= [0.016, 	0.0,	0.0]
		self.aKi				= [0.0952,		0.0,	0.0] #0.00065
		self.aRef				= [self.aDeltaRef[0],	self.aDeltaRef[1],	
									self.aDeltaRef[2]]
	...
	def controlPI(self,xk):
		...
		if (xk<self.aMultiplo[0]*self.aConObj):
		#Modo referencia recta.
			if (xk<200):
				self.aKp[0] 	= 0		#Haga control integral
			else:
				self.aKi[0] 	= 0		#Haga control proporcional
				self.aKp[0]		= 0.2
			ek	= self.aRef[0]-xk
			PI 	= ek*self.aKp[0]+self.aKi[0]*self.aInt[0]
			
		else:
			#Modo referencia en el futuro
			ek	= self.aMasaObj[0]-xk
			if (self.aDoCalcularKp[0]):
				self.aKp[0] 			= self.inRangeCoerce(self.aPWM[0]/(ek),0.2,1)
				self.aInt[0]			= 0
				self.aKi[0]				= 0.09
				self.aDoCalcularKp[0] 	= False
				
			PI 	= ek*self.aKp[0]+self.aKi[0]*self.aInt[0]
			
		PI = self.inRangeCoerce(PI,0,99)
		self.amConPWM.ChangeDutyCycle(PI)

		#Termina algoritmo de control
		#Calcula referencia para el futuro
		self.aRef[0] 			+= self.aDeltaRef[0] 
		self.aInt[0]			+= ek*0.1
	...
	def calcularDeltaReferencia(self,tiempo=15):
		for i in range(3):
			self.aDeltaRef[i] 	= (self.aMultiplo[i]*self.aMasaObj[i])/200
			self.aDeltaRef[i]	= self.inRangeCoerce(self.aDeltaRef[i],10,40)
		#print("Calculadas las referencias")
